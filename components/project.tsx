...
export default function Project({
	...
	return (
		<motion.div
			...
			{" "}
			<section
				className=" bg-gray-100 max-w-[42rem] border border-black/5 rounded-lg overflow-hidden sm:pr-8 relative sm:h-[20rem] even:pl-8
    hover:bg-gray-200 transition sm:group-even:pl-8 dark:text-white dark:bg-white/10 dark:hover:bg-white/20 ">
				<div className="pt-4 pb-7 px-5 sm:pl-10 sm:pr-2 sm:pt-10 sm:max-w-[50%] flex flex-col h-full sm:group-even:ml-[18rem] ">
					...
					<ul className="flex flex-wrap mt-4 gap-2 sm:mt-auto">
						{tags.map((tag, index) => (
							<li
								className="bg-black/[0.7] px-3 py-1 text-[0.7rem] text-white rounded-full dark:text-white/70 "
								key={index}>
								{tag}
							</li>
						))}
				...
				<Image
					src={imageUrl}
					alt="Project Sebastian Worked On"
					quality={95}
